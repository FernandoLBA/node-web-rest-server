...
describe("todo routes testing", () => {
  ...
  test("should return an updated TODO api/todos/:id", async () => {
    ...
    const { body } = await request(testServer.app)
      ...
      .send({ text: "Hola mundo UPDATE", completedAt: "2024-01-12" })
      .expect(200);
  ...
  test("should return 404 if the updated TODO was not found", async () => {
    const { body } = await request(testServer.app)
      .get(`/api/todos/123`)
      .expect(404);

    expect(body).toEqual({ error: "Todo with id 123 is not found" });
  });
